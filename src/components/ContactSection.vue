  ...
  <section id="contact" class="contact-section py-20" ref="sectionRef">
    <v-container style="max-width:1280px">
      <div class="text-center mb-14">
        <div class="section-badge" :class="{ visible: inView }">Get In Touch</div>
        ...
        <p class="section-subtitle mt-4 mx-auto anim-sub" :class="{ visible: inView }" style="max-width:480px">
          Whether it's a question, emergency, or follow-up — our team is available around the clock.
        </p>
      </div>
      ...
      <v-row>
        ...
        >
          ...
          >
            ...
            <div v-for="line in info.lines" :key="line" class="info-line">{{ line }}</div>
          </div>
        </v-col>
      </v-row>
        ...
        <div class="map-inner">
          <div class="map-overlay-info">
            ...
            <div class="map-address">
              ...
              <div class="map-addr-text">123 Healthcare Blvd, Suite 400<br>New York, NY 10001</div>
            </div>
            <v-btn
          ...
          <!-- Fake map grid -->
          <div class="fake-map">
            <div v-for="n in 80" :key="n" class="map-block" :class="{ road: isRoad(n) }" />
          </div>
        </div>
      </div>
    </v-container>
...
.info-card:hover { box-shadow: 0 16px 44px rgba(10,110,97,0.14); transform: translateY(-6px) !important; }
...
.info-title {
  ...
  color: var(--clinic-dark); margin-bottom: 10px;
}
.info-line { font-size: 0.85rem; color: var(--clinic-muted); font-weight: 500; line-height: 1.8; }
...
.map-inner { position: relative; height: 300px; background: #d0e8e3; overflow: hidden; }
...
.map-block { background: #c2dfd8; transition: background 0.3s; }
.map-block.road { background: #e8f5f3; }
...
.map-clinic-name { font-weight: 700; color: var(--clinic-dark); font-size: 0.95rem; }
.map-addr-text   { font-size: 0.8rem; color: var(--clinic-muted); margin-top: 2px; }
